...
class InformacoesClientesCompletosService implements IInformacoesClientesCompletosInterface {
  ...
  @override
  Future<RespostaModel<InformacoesClientesCompletosModel>> buscarInformacoesClientePorId(int id) async {
    RespostaModel<InformacoesClientesCompletosModel> resposta = RespostaModel<InformacoesClientesCompletosModel>();

    try {
      var cliente = await _contexto.from('informacoes_clientes_completos').select().eq("id_cliente", id).single();
  ...
  @override
  Future<RespostaModel<List<InformacoesClientesCompletosModel>>> listarInformacoesClientes() async {
    RespostaModel<List<InformacoesClientesCompletosModel>> resposta = RespostaModel<List<InformacoesClientesCompletosModel>>();

    try {
      var clientes = await _contexto.from('informacoes_clientes_completos').select();

      resposta.Dados = clientes.map<InformacoesClientesCompletosModel>((cliente) => InformacoesClientesCompletosModel(
        id_cliente: cliente['id_cliente'],
        nome: cliente['nome'],
        email: cliente['email'],
        data_nascimento: DateTime.parse(cliente['data_nascimento']),
